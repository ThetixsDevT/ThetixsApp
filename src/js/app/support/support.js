...
app.controller('SupportCtrl', ['$scope', '$state','$http', '$filter', '$modal', 'MyService', 'filterFilter', 'toaster','$timeout',  function($scope,  $state ,$http, $filter,$modal, MyService, filterFilter, toaster,$timeout) {
 ...
 $scope.level=MyService.data.level;
  ...
  $scope.loadSupportTickets= function(){
    $scope.supportTickets=[];
      $scope.questions=[];
    $http.get('https://www.thetixsapp.com:1350/supportTicket').then(function (resp) {   
