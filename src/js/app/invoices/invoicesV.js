...
app.controller('InvoicesVCtrl', ['$scope', '$http', '$filter', '$modal', 'MyService', 'filterFilter', 'datepickerConfig','$state',function($scope, $http, $filter,$modal, MyService,filterFilter, datepickerConfig,$state) {
    ...
    $scope.getProducts = function () {
      ...
      $http.get('https://www.thetixsapp.com:1350/product/?idPartner='+MyService.data.idUser).then(function (resp) {
        ...
        for (var i  = 0; i<$scope.products2.length;i++){
          ...
          var prices="";
          for (var j  = 0; j<$scope.products2[i].itemsPrices.length;j++){
              if ($scope.products2[i].itemsPrices[j].commissionF){
                prices=prices+" "+$scope.products2[i].itemsPrices[j].label+" ----> $ "+$scope.products2[i].itemsPrices[j].commissionF+"<br/>";
                labels=labels+" $ "+$scope.products2[i].itemsPrices[j].commissionF+"<br/>";                
                $scope.products2[i].commissions= prices;
                $scope.products2[i].labels=labels;
              }
              if ($scope.products2[i].itemsPrices[j].commissionP){
                prices=prices+" "+$scope.products2[i].itemsPrices[j].label+" ----> $ "+($scope.products2[i].itemsPrices[j].price*$scope.products2[i].itemsPrices[j].commissionP/100)+"<br/>";
                labels=labels+" $ "+$scope.products2[i].itemsPrices[j].commissionF+"<br/>";  
              }
              $scope.products2[i].commissions=prices;
              $scope.products2[i].labels=labels;
    ...
    $http.get('https://www.thetixsapp.com:1350/product/?idPartner='+MyService.data.idUser).then(function (resp) {
      ...
      for (var i  = 0; i<$scope.products2.length;i++){
          ...
          for (var j  = 0; j<$scope.products2[i].itemsPrices.length;j++){
              if ($scope.products2[i].itemsPrices[j].commissionF){
                // prices=prices+" "+$scope.products2[i].itemsPrices[j].label+" ----> $ "+$scope.products2[i].itemsPrices[j].commissionF+"<br/>";
                                prices=prices+" "+$scope.products2[i].itemsPrices[j].label+"<br/>";
                labels=labels+" $ "+$scope.products2[i].itemsPrices[j].commissionF+"<br/>";                
                $scope.products2[i].commissions= prices;
                $scope.products2[i].labels=labels;
              }
